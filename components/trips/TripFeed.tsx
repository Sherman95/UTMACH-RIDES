  ...
  async function fetchTrips() {
    ...
    try {
      const { data, error } = await supabase
        .from('trips')
        .select('id, driver_id, origin, destination, departure_time, seats_available, price_contribution, status, users(full_name, whatsapp_number, average_rating, total_ratings, total_trips), vehicles(brand, model, color)')
        .eq('status', 'active')
